...
public class StoryPuckScript : MonoBehaviour
{

    public StoryScoreScript ScoreScriptInstance;
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (!WasGoal)
        {
            if (other.tag == "AiGoal")
            {
                ScoreScriptInstance.Increment(StoryScoreScript.Score.PlayerScore);
                WasGoal = true;
                BlueGoal = true;
            ...
            {
                ScoreScriptInstance.Increment(StoryScoreScript.Score.AiScore);
                WasGoal = true;
                audioManager.PlayGoal();
                StartCoroutine(ResetPuck());
            }
